...
$this->title = 'Update Architect: ' . $model->username;
$this->params['breadcrumbs'][] = ['label' => 'Architects', 'url' => ['index']];
$this->params['breadcrumbs'][] = ['label' => $model->username, 'url' => ['view', 'id' => $architect_id, 'user_id' => $model->id]];
...
<ul class="nav nav-tabs">
    <li class="active"><a data-toggle="tab" href="#home">Update Architect</a></li>
    <li><a data-toggle="tab" href="#menu1">Update Profile</a></li>
    <li><a data-toggle="tab" href="#menu2">Update Team</a></li>
    <li><a data-toggle="tab" href="#menu3">Update Info</a></li>
    <li><a data-toggle="tab" href="#menu4">Additional</a></li>
    <li><a data-toggle="tab" href="#trusted_vendors">Trusted Vendors</a></li>
</ul>
<div class="tab-content">
    ...
    <div id="menu1" class="tab-pane fade">
        ...
        <?php echo  $form->field($profilemodel, 'website')->input("url") ?>
        <?php echo  $form->field($profilemodel, 'address') ?>
        <?php echo  $form->field($profilemodel, 'about')->textarea(['rows' => '6']) ?>
        <?php echo  $form->field($profilemodel, 'phone') ?>
        <?php echo  $form->field($profilemodel, 'full_name') ?>
        <?php echo  $form->field($profilemodel, 'city') ?>
        <?php echo  $form->field($profilemodel, 'country') ?>
        ...
        <div class = "form-group">
            <?php if($profilemodel->getAttribute('logo') !=''){?>
                <img id="logofile"  src="../../frontend/web/uploads/architects/<?php echo $architect_id;?>/<?php echo $profilemodel->getAttribute('logo');?>">
            <?php }else {?>
        ...
        <div class = "form-group">
            <?php if($profilemodel->getAttribute('cover') !=''){ ?>
                <img id="coverfile"  src="../../frontend/web/uploads/architects/<?php echo $architect_id;?>/<?php echo $profilemodel->getAttribute('cover');?>" style="max-width: 800px;">
            <?php }else {?>
    ...
    <div id="menu2" class="tab-pane fade">
        ...
        <table role="presentation" class="table table-striped memebrs-table">
            ...
            <?php
            if($ArchitectTeammodel){
                foreach ($ArchitectTeammodel as $row)
                {?>
                    <tr class="template-download fade in img_id_<?= $row->id?>">
                        <td>
                            <p class="name">
                                <input type="text" name="name[<?= $row->id ?>]" value="<?= $row->name ?>" class="form-control">
                            </p>
                        </td>
                        <td>
                            <p class="name">
                                <input type="text" name="designation[<?= $row->id ?>]"  value="<?= $row->designation ?>" class="form-control">
                            </p>
                        </td>
                        <td>
                            <span class="preview">
                                <input type="file" name="picture[<?= $row->id ?>]" >
                            </span>
                        </td>
                        <td>
                            <span class="preview architect-team-img">
                                <a href="../../frontend/web/uploads/user-team/<?= $model->id ?>/<?= $row->picture?>" title="<?= $row->picture?>" download="<?= $row->picture?>" data-gallery="">
                                    <img src="../../frontend/web/uploads/user-team/<?= $model->id ?>/<?= $row->picture?>" >
                                </a>
                            </span>
                        </td>
                        <td>
                            <span class="btn btn-danger delete"  onclick="delete_member('<?= $row->picture?>','<?= $model->id ?>','<?= $row->id ?>')">
    ...
    <div id="trusted_vendors" class="tab-pane fade">
        <h3>Trusted Vendors</h3>
        <div class="form-group paddleftright">
            <label class="control-label" >Select Vendors</label>
            <?php
            echo Html::checkboxList('architect_vendors_id[]', $vend_selection, $vend_row, ['item'=>function ($index, $label, $name, $checked, $value){
...
<script>
    ...
    function add_member() {
        ...
        for( var i=0; i < 5; i++ )
            ran += charset.charAt(Math.floor(Math.random() * charset.length));
        $('.memebrs-table') .append('<tr class="template-download fade in img_id_'+ran+'"><td><p class="name"><input type="text" name="name['+ran+']"  class="form-control"> </p> </td> <td> <p class="name"> <input type="text" name="designation['+ran+']"   class="form-control"> </p> </td> <td> <span class="preview"> <input type="file" name="picture['+ran+']" > </span> </td> <td> </td> <td> <span class="btn btn-danger delete"  onclick="delete_temp_member(&#39;'+ran+'&#39;)"> <i class="glyphicon glyphicon-trash"></i> <span>Delete</span> </span> </td> </tr>');

    }
    function delete_temp_member(ran) {
        $('.img_id_'+ran).remove();
    }
    function delete_member(name, id, row_id) {
        $.ajax({
            url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=architects/member-delete&name='+name+'&row_id='+row_id+'&user_id='+id,
            dataType: 'json',
            type:'POST',
            success: function ( data)
            {
    ...
    function delete_info(name, id, row_id) {
        $.ajax({
            url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=architects/info-delete&id='+row_id+'&user_id='+id,
            dataType: 'json',
            type:'POST',
            success: function ( data)
            {
    ...
    window.onload = function() {
        $('#logouploadmodel-logo').on('change', function(){
            ...
            $.ajax({
                ...
                url: '<?php $ssl = explode('/',$_SERVER['SERVER_PROTOCOL']);
                    echo $ssl[0];?>://<?php echo $_SERVER['SERVER_NAME'];?>/banjaiga/backend/web/index.php?r=architects/logo-cover-upload&id=<?php echo $architect_id;?>&user_id=<?php echo $model->id;?>&image=<?php echo $profilemodel->getAttribute('logo');?>&fieldname=logo',
                dataType : 'json',
        ...
        $('#coveruploadmodel-cover').on('change', function(){
            ...
            $.ajax({
                ...
                data: data,
                url: '<?php $ssl = explode('/',$_SERVER['SERVER_PROTOCOL']);
                    echo $ssl[0];?>://<?php echo $_SERVER['SERVER_NAME'];?>/banjaiga/backend/web/index.php?r=architects/logo-cover-upload&id=<?php echo $architect_id;?>&user_id=<?php echo $model->id;?>&image=<?php echo $profilemodel->getAttribute('logo');?>&fieldname=cover',
                dataType : 'json',
                // in PHP you can call and process file in the same way as if it was submitted from a form:
                // $_FILES['input_file_name']
                success: function(jsonData){
                    ...
                    if(jsonData.crop=='small-image-size-error')
                    {
                        alert("Image is too smalll for cover. Image should be at least 1136px X 388px");
                        $('#coveruploadmodel-cover').show();//
                    }
                    else if(jsonData.crop=='no')
                    {
    ...
    function checkCoordsLogo()
    {
        if (parseInt(jQuery('#lw').val())>0) {
            ...
            $.ajax({
                url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=architects/crop-logo&id=<?php echo $architect_id;?>&user_id=<?php echo $model->id;?>&image='+image+'&lx='+lx+'&ly='+ly+'&lw='+lw+'&lh='+lh,
                dataType: 'json',
                type:'POST',
                success: function ( data)
                {
    ...
    function checkCoordsCover()
    {
        if (parseInt(jQuery('#cw').val())>0) {
            ...
            image = image[image.length-1];
            $.ajax({
                url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=architects/crop-image&id=<?php echo $architect_id;?>&user_id=<?php echo $model->id;?>&image='+image+'&cx='+cx+'&cy='+cy+'&cw='+cw+'&ch='+ch,
                dataType: 'json',
                type:'POST',
                success: function ( data)
                {
