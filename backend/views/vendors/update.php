...
$this->title = 'Update Vendor: ' . $model->username;
$this->params['breadcrumbs'][] = ['label' => 'Vendors', 'url' => ['index']];
$this->params['breadcrumbs'][] = ['label' => $model->username, 'url' => ['view', 'id' => $vendor_id, 'user_id' => $model->id]];
    ...
    <div id="menu1" class="tab-pane fade">
        ...
        <?php echo  $form->field($logouploadmodel, 'logo')->fileInput([ 'accept' => 'image/*']) ?>
            ...
            <div class = "form-group">
                ...
                <img id="logofile"  src="../../frontend/web/uploads/vendors/<?php echo $vendor_id;?>/<?php echo $profilemodel->getAttribute('logo');?>">
                <?php }else {?>
        ...
        <?php echo  $form->field($coveruploadmodel, 'cover')->fileInput([ 'accept' => 'image/*']) ?>
            ...
            <div class = "form-group">
                ...
                <img id="coverfile"  src="../../frontend/web/uploads/vendors/<?php echo $vendor_id;?>/<?php echo $profilemodel->getAttribute('cover');?>" style="max-width: 800px;">
                <?php }else {?>
                    ...
                    <tr class="template-download fade in img_id_<?= $row->id?>">
                        ...
                        <td>
                            <span class="preview vendor-team-img">
                                <a href="../../frontend/web/uploads/user-team/<?= $model->id ?>/<?= $row->picture?>" title="<?= $row->picture?>"  data-gallery="">
            ...
            if($VendorCataloguemodel){
                ...
                {?>
                    <tr class="template-download fade in catalogue_id_<?= $row->id?>">
                        ...
                        <td>
                            <span class="preview vendor-team-img">
                                <a style="color: blue;" href="../../frontend/web/uploads/user-catalogue/<?= $model->id ?>/<?= $row->catalogue?>" title="<?= $row->catalogue?>"  data-gallery="">
                        ...
                        <td>
                            <span class="preview vendor-team-img">
                                <img style="width: 150px;" src="../../frontend/web/uploads/user-catalogue/<?= $model->id ?>/<?= $row->catalogue_image?>"/>
                            </span>
                        </td>
                        <td>
...
<script>
    ...
    function delete_catalogue(name, catalogue_image, user_id, row_id) {
        $.ajax({
            url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=vendors/catalogue-delete&name='+name+'&catalogue_image='+catalogue_image+'&row_id='+row_id+'&user_id='+user_id,
            dataType: 'json',
            type:'POST',
            success: function ( data)
            {
    ...
    function delete_member(name, user_id, row_id) {
        $.ajax({
            url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=vendors/member-delete&name='+name+'&row_id='+row_id+'&user_id='+user_id,
            dataType: 'json',
            type:'POST',
            success: function ( data)
            {
    ...
    window.onload = function() {
        ...
        $('#logouploadmodel-logo').on('change', function(){
            ...
            $.ajax({
                ...
                url: '<?php $ssl = explode('/',$_SERVER['SERVER_PROTOCOL']);
                    echo $ssl[0];?>://<?php echo $_SERVER['SERVER_NAME'];?>/banjaiga/backend/web/index.php?r=vendors/logo-cover-upload&id=<?php echo $vendor_id;?>&user_id=<?php echo $model->id;?>&image=<?php echo $profilemodel->getAttribute('logo');?>&fieldname=logo',
                dataType : 'json',
        ...
        $('#coveruploadmodel-cover').on('change', function(){
            ...
            $.ajax({
                ...
                data: data,
                url: '<?php $ssl = explode('/',$_SERVER['SERVER_PROTOCOL']);
                    echo $ssl[0];?>://<?php echo $_SERVER['SERVER_NAME'];?>/banjaiga/backend/web/index.php?r=vendors/logo-cover-upload&id=<?php echo $vendor_id;?>&user_id=<?php echo $model->id;?>&image=<?php echo $profilemodel->getAttribute('logo');?>&fieldname=cover',
                dataType : 'json',
                // in PHP you can call and process file in the same way as if it was submitted from a form:
                // $_FILES['input_file_name']
                success: function(jsonData){
    ...
    function checkCoordsLogo()
    {
        if (parseInt(jQuery('#lw').val())>0) {
            ...
            $.ajax({
                url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=vendors/crop-logo&id=<?php echo $vendor_id;?>&user_id=<?php echo $model->id;?>&image='+image+'&lx='+lx+'&ly='+ly+'&lw='+lw+'&lh='+lh,
                dataType: 'json',
                type:'POST',
                success: function ( data)
                {
    ...
    function checkCoordsCover()
    {
        if (parseInt(jQuery('#cw').val())>0) {
                ...
                image = image[image.length-1];
            $.ajax({
                url: '<?php echo Yii::$app->request->getBaseUrl(true); ?>?r=vendors/crop-image&id=<?php echo $vendor_id;?>&user_id=<?php echo $model->id;?>&image='+image+'&cx='+cx+'&cy='+cy+'&cw='+cw+'&ch='+ch,
                dataType: 'json',
                type:'POST',
                success: function ( data)
                {
