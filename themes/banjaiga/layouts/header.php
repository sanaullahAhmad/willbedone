    ...
    if ($prj) {
        ...
        if($ShopItems)
        {
            ...
            foreach ($ShopItems as $row)
            {
                if(!$row->is_processed) {
                    ?>
                    <div class="accordion-group accordion-group-list <?php if ($item_count == 0) {
                        echo "active";
                    } ?> shopping_list_item_<?= $row->id ?>">
                        <div class="accordion-heading accordion-heading-list">
                            <a class="accordion-toggle accordion-toggle-style " data-toggle="collapse"
                               data-parent="#accordion2" href="#collapse_<?php echo $item_count; ?>">
                                <!--<img src="<?php /*echo $themeUrl*/
                                ?>/img/sofa-list-icon.png">-->
                                <?php if ($cat = ShoppingListItems::getCategory($row->relatedRecords['products']->product_categories_id, 'category')) {
                                    echo $cat;
                                } else {
                                    echo "Others";
                                }; ?>
                        ...
                        <div id="collapse_<?php echo $item_count; ?>"
                             class="accordion-body collapse  shopping_list_body_<?= $row->id ?>">
                                ...
                                <div class="cart-quote col-md-12">
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <p class="shop-quantity shop-quantity-<?= $row->id ?>"><?php echo $row->quantity ?>
                                                        x</p>
                                                    <span class="shop-quantity-arrows">
                                            <a href="javascript:void(0)"
                                               onclick="update_quantity('up', '<?= $row->id ?>')"> <i
                                                        class="fa fa-angle-up"></i></a>
                                            <a href="javascript:void(0)"
                                               onclick="update_quantity('down', '<?= $row->id ?>')"> <i
                                                        class="fa fa-angle-down"></i></a>
                                        </span>
                                                </div>
                                                <div class="col-md-6 cloase-list">
                                                    <a href="javascript:void(0)"
                                                       onclick="remove_shopping_list_items('<?= $row->id ?>')">
                                        ...
                                        <div class="col-md-12 text-center">
                                            <!--<img src="<?php /*echo $themeUrl*/
                                            ?>/img/table-set.jpg" class="rs-responsive">-->
                                            <img src="<?php echo Yii::$app->request->getBaseUrl(true); ?>/uploads/products-images/<?php echo $row->relatedRecords['products']->id; ?>/<?= pathinfo($row->relatedRecords['products']->featured_image, PATHINFO_FILENAME) . '_446-300.' . pathinfo($row->relatedRecords['products']->featured_image, PATHINFO_EXTENSION) ?>"
                                                 class="rs-responsive1" style="width: 221px;height: 154px">
                                        </div>
                                        <div class="col-md-12 get-quaote">
                                            <div class="row">
                                                <div class="col-md-5 get-quaote-detail">
                                                    ...
                                                    if ($productVendors) {
                                                        $product_vendors_id = $productVendors->vendors_id;
                                                    } else {
                                                        $product_vendors_id = 0;
                                                    }
                                                    $productVendorsDetail = Vendors::find()->where(['id' => $product_vendors_id])->one();
                                                    //
                                                    if ($productVendorsDetail) {
                                                        $productVendorsDetail_userid = $productVendorsDetail->user_id;
                                                    } else {
                                                        $productVendorsDetail_userid = 0;
                                                    }
                                                    $user_info = User::find()->where(['id' => $productVendorsDetail_userid])->one();
                                                    ?>
                                                    <span>by <?php if ($user_info) {
                                                ...
                                                <div class="col-md-7 get-quaote-btn">
                                                    <?php /*if ($row->price_offered != 0) { */?><!--
                                                        <a href="javascript:void(0)">Qoute
                                                            Rs. <?/*= $row->price_offered */?></a>
                                                    --><?php /*} else*/if ($row->status == 0) {
                                                        ...
                                                        <a href="javascript:void(0)" id="get_qoute_link_<?= $row->id ?>"
                                                           onclick="<?= $a_onclick ?>">
                                                            <?= $a_content ?>
                                                        </a>
                                                ...
                                                <div class="col-md-12">
                                                    <?php if ($row->status == 0) { ?>
                                                        <div class="checkbox get-quaote-checkbox">
                                                            <input class="CB2RBVehicles" id="inlineCheckbox12"
                                                                   value="5-marla" name="plot-area" type="checkbox"
                                                                   checked>
                                                            <label for="inlineCheckbox12">Get Quote from all Vendors &
                                                                Manufacturers </label>
                                                        </div>
                                                    <?php } ?>
                                                </div>
            ...
            foreach ($ShopItems as $row)
            {
                if($row->is_processed && $row->status!=5)//Marked Processed by Admin and Not declined by user.
                {
                $seconds = strtotime($row->date_proccessed. ' +10 days') - time();
                ...
                $seconds %= 60;
                ?>
                <div class="accordion-group accordion-group-list quotes-box" <?php if($item_count%2==0){?> style="background: #9e9e9e !important; " <?php }?>>
                    <div class="accordion-heading accordion-heading-list">
                        <a class="accordion-toggle accordion-toggle-style " data-toggle="collapse" data-parent="#accordion3" href="#collapse_<?php echo $item_count; ?>">
                            <ul class="quotes-time">
                                <li>DAYS<span><?= $days?></span></li>
                                <li>HOURS<span><?= $hours?></span></li>
                                <li>MINS<span><?= $minutes?></span></li>
                            </ul>
                            <span class="my-quotes"> <?= $row->relatedRecords['products']->name?></span>
                        </a>
                    </div>
                            ...
                            <div class="total-cast-box">
                                <div class="total-cast-box-inner">
                                    ...
                                    <div class="col-md-8 total-cast-box-detail">
                                        ...
                                        <h4>Total Cost <span>PKR <?= number_format($row->price_offered)?> X <?= $row->quantity?> = <?= number_format($row->price_offered*$row->quantity)?></span></h4>
                                    </div>
                                    <div class="col-md-12 total-cast-box-btn">
                                        <a href="javascript:void(0)" onclick="save_shopping_list(<?= $row->id;?>, 3)" class="total-cast-btn save_list_button" style="display: <?php if($row->status==2){?>inline-block<?php }else{?>none<?php }?>;">
                                            <i class="fa fa-heart" aria-hidden="true"></i>
                                            SAVE
                                        </a>
                                        <a href="javascript:void(0)" onclick="save_shopping_list(<?= $row->id;?>, 5)" class="total-cast-btn list_decline" style="display: <?php if($row->status==2){?>inline-block<?php }else{?>none<?php }?>;">
                                            <i class="fa fa-heart" aria-hidden="true"></i>
                                            decline
                                        </a>
                                        <a href="javascript:void(0)" onclick="save_shopping_list(<?= $row->id;?>, 6)" class="total-cast-btn place_order_button" style="display: <?php if($row->status==3){echo "inline-block";}else{ echo "none";}?>;">
                                            <i class="fa fa-heart" aria-hidden="true"></i>
                                            Place Order
                                        </a>
                                        <a href="javascript:void(0)"  class="total-cast-btn order_placed_button" style="display: <?php if($row->status==6){echo "inline-block";}else{ echo "none";}?>;">
                                            <i class="fa fa-heart" aria-hidden="true"></i>
                                            Order Placed
                                        </a>
                                        <a href="#" class="list_visit_showroom"  onclick="save_shopping_list(<?= $row->id;?>, 4)" style="display: <?php if($row->status==3){echo "inline-block";}else{ echo "none";}?>;">
                                            Visit Showroom
                                        </a>
                                        <a href="#" class="" style="display: <?php if($row->status==9){echo "inline-block";}else{ echo "none";}?>;">
                                            Showroom Visited
                                        </a>
        ...
        <nav class="navbar navbar-inverse top-nav-bar">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="<?php Yii::$app->request->getBaseUrl(true);?>?r=site">
                    <img src="<?php echo $themeUrl; ?>/img/logo-and-text_01_beta.png" >
                </a>
            </div>
            <div class="collapse navbar-collapse js-navbar-collapse">
                <ul class="nav navbar-nav main-mega-menu">
                    ...
                    <li class="dropdown mega-dropdown"> <a href="#" >Professionals <i class="fa fa-angle-down"></i></a>
